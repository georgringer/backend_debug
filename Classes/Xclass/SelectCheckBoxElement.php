...
class SelectCheckBoxElement extends \TYPO3\CMS\Backend\Form\Element\SelectCheckBoxElement
{
            ...
            foreach ($groups as $groupKey => $group) {
                ...
                if (is_array($group['items']) && !empty($group['items'])) {
                    ...
                    foreach ($group['items'] as $item) {
                        ...
                        // Xclass start
                        $tableRows[] =        '<label class="label-block" for="' . $item['id'] . '">' . htmlspecialchars(FormElementUtility::appendValueToLabelInDebugMode($item['title'], $item['value']), ENT_COMPAT, 'UTF-8', false) . '</label>';
                        // Xclass end
                        $tableRows[] =    '</td>';
